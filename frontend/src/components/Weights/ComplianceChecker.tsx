...
export default function ComplianceChecker({
  ...
  const [axleWeights, setAxleWeights] = useState<number[]>(Array(initialAxleCount).fill(initialGrossWeight / initialAxleCount));
  const [axleSpacing, setAxleSpacing] = useState<number[]>(Array(initialAxleCount - 1).fill(4.5));
  const [totalLength, setTotalLength] = useState(axleSpacing.reduce((sum, spacing) => sum + spacing, 0));
            ...
            <div className="bg-gray-800 rounded-lg p-4 border border-gray-700">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <p className="text-lg text-white">{formatWeight(complianceResult.maxAllowedWeight)}</p>
                </div>
                
                <div>
                  <p className="text-sm text-gray-400">Current Gross Weight</p>
                  <p className={`text-lg ${complianceResult.overWeight > 0 ? 'text-red-400' : 'text-white'}`}>
                    ...
                    {complianceResult.violations.map((violation, index) => (
                      <li key={index} className="flex items-start text-sm">
                        ...
                        <span className="text-gray-300">
                          {violation.type}: {formatWeight(violation.actual)} exceeds limit of {formatWeight(violation.limit)}
                          {violation.axleIndex !== undefined && ` (Axle ${violation.axleIndex + 1})`}
                        </span>
                      </li>
                    ))}
                  </ul>
