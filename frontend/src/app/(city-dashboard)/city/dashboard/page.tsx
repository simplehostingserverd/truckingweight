              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">{dashboardData.totalWeighings.toLocaleString()}</div>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">${dashboardData.revenueCollected.toLocaleString()}</div>
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">{dashboardData.activePermits.toLocaleString()}</div>
                      <p className="text-xs text-gray-500">
                        <span className="text-yellow-500">{dashboardData.pendingPermits}</span> pending
                      </p>
                    </div>
                  </div>
                )}
              ...
              <CardContent>
                {isLoading ? (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      <div className="text-2xl font-bold text-white">{dashboardData.recentViolations.toLocaleString()}</div>
                  ...
                  <table className="w-full text-sm text-left text-gray-300">
                    ...
                    <tbody>
                      {recentWeighings.map((weighing: any) => (
                        <tr key={weighing.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                          ...
                          <td className="px-4 py-3">{weighing.grossWeight.toLocaleString()} lbs</td>
                          <td className="px-4 py-3">{new Date(weighing.weighDate).toLocaleDateString()}</td>
                          <td className="px-4 py-3">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                              ...
                              {weighing.status === 'Compliant' && <CheckCircleIcon className="w-3 h-3 mr-1" />}
                              {weighing.status === 'Non-Compliant' && <XCircleIcon className="w-3 h-3 mr-1" />}
                              {weighing.status === 'Warning' && <ExclamationCircleIcon className="w-3 h-3 mr-1" />}
                              {weighing.status}
                            </span>
                          </td>
                        </tr>
                    ...
                    <div className="relative">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div className="text-center">
                          <div className="text-3xl font-bold text-white">{dashboardData.complianceRate}%</div>
              ...
              {isLoading ? (
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <LineChart data={revenueChartData}>
                    ...
                    />
                    <Legend wrapperStyle={{ color: '#f9fafb' }} />
                    <Line type="monotone" dataKey="Permit Revenue" stroke="#4ade80" strokeWidth={2} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey="Fine Revenue" stroke="#ef4444" strokeWidth={2} dot={{ r: 4 }} />
                    <Line type="monotone" dataKey="Total" stroke="#60a5fa" strokeWidth={2} dot={{ r: 4 }} />
                  </LineChart>
                </ResponsiveContainer>
              )}
            </CardContent>
