  ...
  while ((match = modelRegex.exec(prismaSchema)) !== null) {
    ...
    for (const line of lines) {
      ...
      if (trimmedLine.includes('@relation')) {
        ...
        const relationMatch = trimmedLine.match(/(\w+)\s+(\w+).*@relation\(fields:\s*\[(\w+)\],\s*references:\s*\[(\w+)\]/);
        if (relationMatch) {
          const [, relatedType, fieldName, localField, foreignField] = relationMatch;
          constraints.push(`FOREIGN KEY (${localField}) REFERENCES ${relatedType}(${foreignField})`);
        }
        continue;
      }

      // Parse regular field
      const fieldMatch = trimmedLine.match(/(\w+)\s+(\w+)(\??)(\s+@id)?(\s+@default\(([^)]+)\))?(\s+@db\.(\w+)(\(([^)]+)\))?)?/);
      if (fieldMatch) {
        const [, fieldName, fieldType, optional, isId, , defaultValue, , dbType, , dbTypeParams] = fieldMatch;
