...
export interface TollProviderEnvironmentConfig {
  pcmiler: {
    apiKey?: string;
    baseUrl: string;
    region: string;
 ...
 */
export function loadTollConfig(): TollProviderEnvironmentConfig {
  const config: TollProviderEnvironmentConfig = {
    pcmiler: {
  ...
  logger.info('Toll provider configuration loaded:', {
 ...
 */
export function isTollProviderConfigured(providerName: string): boolean {
  const config = loadTollConfig();
 ...
 */
export function getEnabledTollProviders(): string[] {
  const config = loadTollConfig();
 ...
 */
export function validateTollConfig(): { valid: boolean; errors: string[] } {
  const config = loadTollConfig();
  const errors: string[] = [];
  ...
  if (enabledProviders.length === 0) {
    logger.warn('No toll providers are configured. Toll management features will be disabled.');
  }
 ...
 */
export function initializeTollConfig(): void {
  const validation = validateTollConfig();
  
  if (!validation.valid) {
    logger.error('Toll configuration validation failed:', validation.errors);
    throw new Error(`Toll configuration errors: ${validation.errors.join(', ')}`);
  }
  
  const enabledProviders = getEnabledTollProviders();
  logger.info(`Toll management initialized with ${enabledProviders.length} providers:`, enabledProviders);
}
